...
namespace WindowsFormsApp28
{
    public partial class Form1 : Form
    {
...

        {
            ...
            if (!lib.ContainsKey((int)numericUpDown1.Value) && textBox1.Text!="")//lib.ContainsKey((int)numericUpDown1.Value)
            {
               
                lib.Add((int)numericUpDown1.Value, new JerseyNumber(textBox1.Text, (int)numericUpDown2.Value));

                comboBox1.Items.Add((int)numericUpDown1.Value);
            }
            else
            {
                MessageBox.Show("Игрок с таким номером есть или не все поля заполнены !");
            }
        }
        ...
        {
            int key = 0;
            if (lib.ContainsKey((int)numericUpDown3.Value))
            {
                key = (int)numericUpDown3.Value;
                lib.Remove(key);
                comboBox1.Items.Remove(key);
            }
            else
            {
                MessageBox.Show("Игрока с таким номером нет!");
            }
